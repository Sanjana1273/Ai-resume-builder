...
function Dashboard() {
  const colors = ["#9333ea","#d97706","#dc2626" , "#0284cf","#16a34a"]
  ...
  return (
    <div>
      <div className='maxx-w-7xl mx-auto px-4 py-8'>
        ...
        <div className='flex gap-4'>
          ...
          duration-300 cursor-pointer'>
            <PlusIcon size={44} className='transition-all duration-300 p-2.5
          ...
          duration-300 cursor-pointer'>
            <UploadCloudIcon size={44} className='
      ...
      {allResume.map((resume , index)=>{
       ...
       return (
        <button key={index} onClick={()=> navigate(`/app/builder/${resume._id}`)} className='relative w-full sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 border group
        hover:shadow-lg transition-all duration-300 cursor-pointer' style={{
          ...
          group-hover:text-slate-500 transition-all duration-300 px-2 text-center' style={{color:baseColor +'90'}}> updated on {new Date(resume.updatedAt).toLocaleDateString()}</p>
          ...
          <div onClick={e=> e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
            ...
            <div>
              <label htmlFor='resume-input' className='block text-sm text-slate-700'>
                ...
                border group text-slate-400 border-slate-400 border-dashed rounded-mb p-4 py-10 my-4  hover:border-green-500 
                hover:text-green-700 cursor-pointer transition-colors'>
                  {resume ? (
                    <p className='text-green-700'>{resume.name}</p>
                  ):(
                    <>
                    <UploadCloudIcon  className='size-15 stroke-1' />
                    <p>Upload resume</p>
                    </>
                  )}
              ...
              <input type='file' id='resume-input' accept='.pdf' hidden onChange={()=> setResume(e.target.files[0])} />
            </div>
